...
from_string :: proc(date: string) -> (out: Date, err: DateError) {
    ...
    if len(date) >= 5 && date[4:5] == "-" {
        ...
        if len(date) > 10 {
            ...
            date = date[11:]
            if len(date) == 0 do return out, .FAILED_AT_TIME_SEPERATOR
            expect_time = true
        }
    }
    ...
    if len(date) >= 5 && date[2] == ':' {
        ...
        if len(date) > 0 && date[0] == ':' {
            date = date[1:]
            if len(date) < 2 || !are_all_numbers(date[:2]) do return out, .FAILED_AT_SECOND
            ...
            if offset != -1 do date = date[offset:]
            else do date = ""
        }

        if date == "" {
            out.flags += {.local_date}
            return
        }

        if date[:1] == "z" || date[:1] == "Z" do return

        if len(date) < 6 do return out, .FAILED_AT_OFFSET_HOUR
...
partial_date_to_string :: proc(date: Date, time_sep := ' ') -> (out: string, err: DateError) {
    ...
    if .local_date in date.flags do return strings.to_string(b), .NONE
...
is_date_lax :: proc(date: string) -> bool {
    if len(date) >= 10 && date[4] == '-' && date[7] == '-' {     // date or datetime
        return are_all_numbers(date[0:4]) && are_all_numbers(date[5:7]) && are_all_numbers(date[8:10]) && true
    }

    // time (with or without seconds and with optional offset)
    if len(date) >= 5 && date[2] == ':' {
        return are_all_numbers(date[0:2]) && are_all_numbers(date[3:5]) && true
    }
...
from_odin_datetime :: proc(dt: datetime.DateTime, utc_offset: int) -> Date {
    ...
    return {
        ...
        second = f32(dt.second) + f32(dt.nano) / 1e9,
        offset_hour = utc_offset / 60,
        offset_minute = utc_offset % 60,
        flags = (({.date_only} if date_only else {}) | ({.time_only} if time_only else {})),
    }
}
