...
print_error :: proc(err: Error, alloc := context.allocator) -> (fatal: bool) {
    ...
    err := err
    err.formatted.buf = make(type_of(err.formatted.buf), alloc)
    b_printf(&err.formatted, "%s:%d %s! %s\n", err.file, err.line + 1, descriptions[err.type], err.more.buf[:])

    message := string(err.formatted.buf[:])
    if message != "" {
        spy.error(message)
        delete(message, alloc)
    }
    return true
}
...
skip_newline :: proc(io: ^IO) -> (ok: bool) {
    ok = peek(io) == "\n"
    for peek(io) == "\n" {
        io.err.line += 1
        skip(io)
...
// #+vet redundancy public-api
validate :: proc(raw_tokens: []string, file: string, alloc := context.allocator) -> Error {
    io: IO = {
        toks = raw_tokens,
...
validate_stmt :: proc(io: ^IO) -> bool {
    return(
        skip_newline(io) ||
        (validate_array(io) || validate_table(io) || validate_assign(io)) &&
            !err_if_not(
                    io,
                    peek(io) == "" || peek(io) == "\n",
                    .Missing_Newline,
                    "Found a missing new line between statements!",
                ) \
    )
}
...
validate_path :: proc(io: ^IO) -> bool {     //{{{
    validate_name :: proc(io: ^IO) -> bool {
        ...
        case 1:
            ...
            if strings.contains_rune(inner, '\n') || strings.contains_rune(inner, '\r') do return false
        case:
            return false
        }
...
validate_string :: proc(io: ^IO) -> bool {     //{{{
    validate_quotes :: proc(io: ^IO) -> bool {
        token := peek(io)
        qcount := get_quote_count(token)
        if qcount == 0 do return false

        if qcount == 1 {
            inner := token[1:len(token) - 1]
            if strings.contains_rune(inner, '\n') || strings.contains_rune(inner, '\r') {
                make_err(io, .Bad_Value, "Single-line strings cannot contain raw newlines")
                return false
            }
        } else if qcount == 3 {
            // valid multiline string
        } else {
            make_err(io, .Missing_Quote, "string '%s' is missing one or more quotes!", token)
            return false
        }

        parsed, err := unquote(token)
        if err.type != .None {
            io.err.type = err.type
            io.err.more = err.more
            return false
        }
        defer decoded_string_destroy(&parsed)
        skip(io)
        return true
    }
...
validate_date :: proc(io: ^IO) -> (ok: bool) {     //{{{
    if !dates.is_date_lax(peek(io)) do return false
    ...
    if peek(io) == "." {
        frac := peek(io, 1)
        if len(frac) == 0 || !is_digit(rune(frac[0]), 10) {
            make_err(io, .Bad_Date, "Bad millisecond count in the date!")
            return false
        }
    ...
    _, err := dates.from_string(strings.to_string(full))
    if err != .NONE {
        make_err(io, .Bad_Date, "The date: '%s' is not valid, please use rfc 3339!", strings.to_string(full))
        return false
    }
    return true
} //}}}
...
validate_number :: proc(io: ^IO) -> bool {     //{{{
    ...
    number := peek(io)
    had_sign := at(number, 0) == '+' || at(number, 0) == '-'
    if had_sign do number = number[1:]
    ...
    if eq(number, "nan") || eq(number, "inf") {
        err_if_not(
            ...
            .Bad_Float,
            "NaN and Inf must be fully lowercase in TOML: `nan` and `inf`! (I don't know why). Your's is: '%s'!",
            peek(io),
        )
        skip(io)
        return true
    ...
    validate_underscores :: proc(io: ^IO, r: rune, p: rune, is_last: bool) -> bool {
        ...
        switch {
        case p == '_':
            make_err(io, .Bad_Integer, "Double underscore mid number!")
        case p == 0:
            make_err(io, .Bad_Integer, "Underscore cannot be the first character in a number!")
        case is_last:
            make_err(io, .Bad_Integer, "Underscore cannot be the last character in a number!")
        case:
            return true
        }
        return false
...

    {
        ...
        if exp1 == "" && strings.contains_any(number, "eE") {
            make_err(io, .Bad_Float, "Trailing exponent marker in number.")
            return false
        }
        if peek(io, 1) == "." {
            if exp1 != "" {
                make_err(io, .Bad_Float, "A number cannot have both decimal part and exponent split like this.")
                return false
            }
            fraction, exp2 = split_by(peek(io, 2), "eE")
            if exp2 == "" && strings.contains_any(peek(io, 2), "eE") {
                make_err(io, .Bad_Float, "Trailing exponent marker in number.")
                return false
            }
        ...
        if exponent == "" && (exp1 != "" || exp2 != "") {
            make_err(io, .Bad_Float, "Exponent must include at least one digit.")
            return false
        }
        if main == "" && fraction != "" {
            make_err(io, .Bad_Float, "Floats must have an integer part before '.'.")
            return false
        }
    }
    ...
    if at(main, 0) == '0' {
        ...
        case 'x':
            if had_sign {
                make_err(io, .Bad_Integer, "Hex integers cannot have an explicit sign.")
                return false
            }
            base = 16; main = main[2:]
        case 'o':
            if had_sign {
                make_err(io, .Bad_Integer, "Octal integers cannot have an explicit sign.")
                return false
            }
            base = 8; main = main[2:]
        case 'b':
            if had_sign {
                make_err(io, .Bad_Integer, "Binary integers cannot have an explicit sign.")
                return false
            }
            base = 2; main = main[2:]
        case 0: // nothing
        case:
            make_err(io, .Bad_Integer, "A number cannot start with '0'. Please use '0o1234' for octal!")
        }
    }
