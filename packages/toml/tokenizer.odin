...
tokenize :: proc(raw: string, file := "<unknown file>") -> (tokens: [dynamic]string, err: Error) {
    ...
    // Reject byte streams that are not valid UTF-8.
    for i := 0; i < len(raw); {
        r, size := utf8.decode_rune_in_string(raw[i:])
        if r == utf8.RUNE_ERROR && size == 1 && raw[i] >= 128 {
            return tokens, set_err(&err, .Bad_Unicode_Char, "Invalid UTF-8 at byte %d", i)
        }
        if size <= 0 do break
        i += size
    }
