...
@(private)
Decoded_String :: struct {
    text:  string,
    owned: bool,
}

@(private)
decoded_string_make_owned :: proc(value: ^Decoded_String) {
    if value.owned do return
    value.text = strings.clone(value.text)
    value.owned = true
}

@(private)
decoded_string_destroy :: proc(value: ^Decoded_String) {
    if value.owned {
        delete_string(value.text)
    }
    value^ = {}
}
...
cleanup_backslashes :: proc(
    ...
    if literal do return {text = str}, err
    ...
    if !has_backslash do return {text = str}, err
            ...
            case 'x', 'u', 'U':
                ...
                if len(raw) < i + to_skip + 1 {
                    set_err(
                        &err,
                        .Bad_Unicode_Char,
                        "'\\%v' must have %v hex digits after it in string:",
                        r,
                        to_skip,
                        raw,
                    )
    ...
    return {text = strings.clone(strings.to_string(b)), owned = true}, err
}
...
// @(private)
// between_any :: proc(a, b: rune) -> bool {
//     for i := 0; i < len(b); i += 2 {
...
get_quote_count :: proc(a: string) -> int {
    s := len(a)
    if s > 2 && ((a[:3] == "\"\"\"" && a[s - 3:] == "\"\"\"") || (a[:3] == "'''" && a[s - 3:] == "'''")) { return 3 }

    if s > 0 && ((a[:1] == "\"" && a[s - 1:] == "\"") || (a[:1] == "'" && a[s - 1:] == "'")) { return 1 }

    return 0
}

@(private)
unquote :: proc(a: string, fluff: ..any) -> (result: Decoded_String, err: Error) {
    if len(a) == 0 {
        err.type = .Bad_Name
        strings.write_string(&err.more, "Empty key is invalid in this context.")
        return {}, err
    }
    ...
    return cleanup_backslashes(unquoted, a[0] == '\'', qcount == 3 && a[0] == '"')
}
...
toml_ucs_to_utf8 :: proc(code: u64) -> (buf: [6]u8, byte_count: int) {
    ...
    if code > 0x10FFFF do return buf, -1
