...
gui_circular_progress :: proc(ctx: ^Gui_Context, label: string, value: f32) {
    ...
    gui_text_clipped(
        ctx,
        {bounds.x + size + 18, bounds.y, max(bounds.w - size - 18, 0), bounds.h},
        {bounds.x + size + 24, bounds.y + 28},
        label,
        ctx.style.text,
    )
}
...
gui_stepper_button_at :: proc(
    ...
    if direction < 0 {
        gui_line(
            ctx,
            {center.x + x, center.y - size},
            {center.x - x, center.y},
            ctx.style.text_muted,
            ctx.style.border_width * 2,
        )
        gui_line(
            ctx,
            {center.x - x, center.y},
...
gui_select_chrome :: proc(ctx: ^Gui_Context, bounds: Rect, display: string, id: Gui_Id, open, focused: bool) {
    ...
    gui_shadow(
        ...
        ctx.style.shadow_blur * 0.72,
        {0, 0, 0, 0.18},
    )
    gui_round_rect(ctx, bounds, ctx.style.radius_control, fill)
    gui_round_stroke(ctx, bounds, ctx.style.radius_control, border, stroke_width)
    text_rect := gui_inset_edges(
        bounds,
        {left = ctx.style.control_padding * 1.5, top = 0, right = bounds.h, bottom = 0},
    )
    gui_text_clipped(
        ctx,
        text_rect,
        {text_rect.x, bounds.y + max((bounds.h - ctx.style.body_text_height) * 0.5, 0)},
        display,
        ctx.style.text,
    )
...
gui_combo_popup_rect :: proc(
    ctx: ^Gui_Context,
    bounds: Rect,
    options: []string,
    query: string,
    match_count, selected_index: int,
) -> Rect {
    ...
    max_width :=
        f32(ctx.input.window_width) > 0 ? max(f32(ctx.input.window_width) - ctx.style.spacing_1 * 2, bounds.w) : width
...
gui_combobox_keyed :: proc(
    ctx: ^Gui_Context,
    label, key: string,
    current: ^int,
    options: []string,
    query_buffer: []u8,
    focus_accept_opens := true,
) -> bool {
    if len(options) == 0 {
        return false
    }
    ...
    clicked :=
        (focus_accept_opens && control.focused && accept_pressed) ||
        (control.hovered && ctx.active == id && ctx.input.mouse_released)
    ...
    for match in matches {
        ...
        if visible_row &&
           !opened_this_frame &&
           gui_pointer_enabled(ctx) &&
           gui_contains(list_viewport, ctx.input.mouse_pos) &&
           gui_contains(row, ctx.input.mouse_pos) {
            ctx.hot = row_id
    ...
    for option, match in ctx.combo_popup_options {
        ...
        if y + ctx.style.row_height > list_viewport.y && y < list_viewport.y + list_viewport.h {
            ...
            if match == ctx.combo_highlight {
                gui_rect(
                    ctx,
                    {row.x + 3, row.y + 7, max(ctx.style.border_width * 2, 2), max(row.h - 14, 1)},
                    ctx.style.accent,
                )
            }
            gui_text_clipped(ctx, gui_inset_edges(row, {
                    left   = ctx.style.control_padding * 1.5,
                    top    = 0,
                    right  = gui_scrollbar_reserved_width(ctx) + ctx.style.control_padding,
                    bottom = 0,
                }), {text_left, row.y + max((row.h - ctx.style.body_text_height) * 0.5, 0)}, option, ctx.style.text)
        }
        y += ctx.style.row_height
        if y >= popup.y + popup.h {
            break
