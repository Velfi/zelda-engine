...
gui_label :: proc(ctx: ^Gui_Context, text: string) {
	...
	gui_text_clipped(ctx, bounds, {bounds.x + gui_space(ctx.style, .Quarter), bounds.y + gui_text_inset_y(ctx.style, .Body, bounds.h)}, text, ctx.style.text_muted)
}
...
gui_heading :: proc(ctx: ^Gui_Context, text: string) {
	...
	append(&ctx.commands, Draw_Command{
		kind = .Text,
		rect = {bounds.x + gui_space(ctx.style, .Quarter), bounds.y + gui_text_inset_y(ctx.style, .Heading, bounds.h), 0, 0},
...
gui_text_block :: proc(ctx: ^Gui_Context, text: string, max_width: f32, color: Color) {
	...
	gui_scissor_begin(ctx, bounds)
	gui_text_wrapped_at(ctx, {bounds.x + inline_inset, bounds.y + gui_text_line_inset_y(ctx.style, .Body)}, text, wrap_width, color)
	gui_scissor_end(ctx)
}
