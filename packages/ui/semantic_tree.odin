...
gui_semantic_container_begin :: proc(
    ctx: ^Gui_Context,
    kind: Gui_Semantic_Node_Kind,
    bounds: Rect,
    id: Gui_Id = GUI_ID_NONE,
) {
...
gui_semantic_finalize :: proc(ctx: ^Gui_Context) {
    ...
    for retry in 0 ..< GUI_LAYOUT_RETRY_LIMIT {
        ...
        for i in 0 ..< ctx.semantic_node_count {
            ...
            valid :=
                node.bounds.x == node.bounds.x &&
                node.bounds.y == node.bounds.y &&
                node.bounds.w >= 0 &&
                node.bounds.h >= 0
            node.unstable = !valid
            if !valid {
                unstable += 1
                if node.id != GUI_ID_NONE &&
                   ctx.next_semantic_unstable_id_count < len(ctx.next_semantic_unstable_ids) {
