...
Ui_Document_Runtime_Binding :: struct {
    ...
    enum_options:        []string,
    userdata:            rawptr,
    draw_slot:           proc(_: rawptr, _: ^Gui_Context, _: Rect),
...
ui_document_runtime_binding :: proc(
    bindings: []Ui_Document_Runtime_Binding,
    id: string,
) -> (
    ^Ui_Document_Runtime_Binding,
    bool,
) {
    for _, i in bindings {
        binding := &bindings[i]
...
ui_document_validate_bindings :: proc(
    document: ^Ui_Document,
    bindings: []Ui_Document_Runtime_Binding,
) -> Ui_Document_Validation {
...
ui_document_draw :: proc(
    document: ^Ui_Document,
    ctx: ^Gui_Context,
    bounds: Rect,
    bindings: []Ui_Document_Runtime_Binding,
    actions: ^Ui_Document_Action_State,
) -> Ui_Document_Validation {
...
ui_document_constraint_write :: proc(rect: ^Rect, property: Ui_Document_Constraint_Property, value: f32) {
    ...
    case .Bottom:
        rect.y = value - rect.h
    case .Center_X:
        rect.x = value - rect.w * 0.5
    case .Center_Y:
        rect.y = value - rect.h * 0.5
    }
}
...
ui_document_solve_root_bounds :: proc(document: ^Ui_Document, viewport: Rect) -> Rect {
    ...
    for _ in 0 ..< 2 {
        for constraint in document.constraints {
            ...
            case "equal":
                ui_document_constraint_write(&result, constraint.left_property, target)
            case "less_equal":
                if current > target do ui_document_constraint_write(&result, constraint.left_property, target)
            case "greater_equal":
                if current < target do ui_document_constraint_write(&result, constraint.left_property, target)
            }
        }
...
ui_document_draw_node :: proc(
    ...
    case .Stack, .Row, .Grid, .Overlay, .Anchor:
        ...
        #partial switch node.kind {
        case .Grid:
            ctx.semantic_next_container_kind = .Grid
        case .Overlay, .Anchor:
            ctx.semantic_next_container_kind = .Overlay
        case .Row:
            ctx.semantic_next_container_kind = .Row
        case:
            ctx.semantic_next_container_kind = .Stack
        }
    ...
    case .Scroll:
        ...
        if content_height <= 0 {
            for child in document.nodes {
                ...
                if binding, ok := ui_document_runtime_binding(bindings, child.source.binding);
                   ok && binding.kind == .Slot {
    ...
    case .Text:
        ...
        if binding, ok := ui_document_runtime_binding(bindings, node.source.binding);
           ok && binding.kind == .String && binding.string_value != nil {
            text = binding.string_value^
        }
        gui_label(ctx, text)
    case .Image:
        if binding, ok := ui_document_runtime_binding(bindings, node.source.binding);
           ok && binding.kind == .Image && binding.image_value != nil {
            rect := gui_next_rect(
                ctx,
                height = node.source.layout.height > 0 ? node.source.layout.height : ctx.style.row_height * 3,
            )
    ...
    case .Selector:
        if binding, ok := ui_document_runtime_binding(bindings, node.source.binding);
           ok && binding.integer_value != nil && len(binding.enum_options) > 0 {
    ...
    case .Slot:
        if binding, ok := ui_document_runtime_binding(bindings, node.source.binding);
           ok && binding.kind == .Slot && binding.draw_slot != nil {
            rect := bounds
            if !root {
...
ui_document_draw_children :: proc(
    document: ^Ui_Document,
    parent: int,
    ctx: ^Gui_Context,
    bounds: Rect,
...
ui_document_scroll_value :: proc(ctx: ^Gui_Context, document_id, element_id: string) -> ^f32 {
    ...
    ctx.document_scroll_slots[index] = {
        id         = id,
...
ui_document_constraint_reference :: proc(
    document: ^Ui_Document,
    reference: string,
    allow_viewport: bool,
) -> (
    node: int,
    property: Ui_Document_Constraint_Property,
    viewport: bool,
    ok: bool,
) {
    dot := -1
...
ui_document_validate_and_compile :: proc(document: ^Ui_Document, allocator: mem.Allocator) -> Ui_Document_Validation {
    ...
    for node, i in document.nodes {
        if len(node.source.binding) > 0 {
            ...
            kind_matches :=
                node.kind == .Text && expected == .String ||
    ...
    for constraint, i in document.source.constraints {
        if len(constraint.left) == 0 ||
           (constraint.relation != "equal" &&
                   ...
                   constraint.relation != "greater_equal") ||
        ...
        right_node, right_property, right_viewport, right_ok := ui_document_constraint_reference(
            document,
            constraint.right,
            true,
        )
        ...
        append(
            ...
            Ui_Document_Constraint{constraint, left_node, left_property, right_node, right_property, right_viewport},
        )
    }
