...
gui_stroke :: proc(ctx: ^Gui_Context, rect: Rect, color: Color) {
    append(
        &ctx.commands,
        Draw_Command{kind = .Stroked_Rect, rect = rect, color = color, stroke_width = ctx.style.border_width},
    )
}
...
gui_round_stroke :: proc(ctx: ^Gui_Context, rect: Rect, radius: f32, color: Color, width: f32) {
    append(
        &ctx.commands,
        Draw_Command{kind = .Stroked_Rounded_Rect, rect = rect, color = color, radius = radius, stroke_width = width},
    )
}
...
gui_gradient_round_rect :: proc(ctx: ^Gui_Context, rect: Rect, radius: f32, top, bottom: Color) {
    append(
        &ctx.commands,
        Draw_Command{kind = .Gradient_Rect, rect = rect, color = top, color_2 = bottom, radius = radius},
    )
}
...
gui_shader_rect :: proc(ctx: ^Gui_Context, rect: Rect, kind: Gui_Shader_Kind, params: Color, tint: Color) {
    append(
        &ctx.commands,
        Draw_Command{kind = .Shader_Rect, rect = rect, color = tint, shader_kind = kind, shader_params = params},
    )
}
...
gui_ellipse_stroke :: proc(ctx: ^Gui_Context, rect: Rect, color: Color, width: f32) {
    append(
        &ctx.commands,
        Draw_Command{kind = .Stroked_Ellipse, rect = rect, color = color, stroke_width = max(width, 1)},
    )
}
...
gui_image_uv_filtered_blend :: proc(
    ctx: ^Gui_Context,
    rect: Rect,
    image_id: Gui_Image_Id,
    tint: Color,
...
gui_refractive_glass_rect :: proc(ctx: ^Gui_Context, rect: Rect, style: Gui_Glass_Style) {
    ...
    append(
        &ctx.commands,
        Draw_Command{kind = .Refractive_Glass_Rect, rect = rect, color = glass.tint, glass_style = glass},
    )
}
...
gui_focus_ring :: proc(ctx: ^Gui_Context, rect: Rect) {
    ...
    gui_round_stroke(
        ctx,
        outer,
        ctx.style.radius_control + ctx.style.focus_ring_width,
        gui_apply_opacity(ctx.style.accent, 0.86),
        ctx.style.focus_ring_width,
    )
}
...
gui_focus_or_edit_ring :: proc(ctx: ^Gui_Context, id: Gui_Id, rect: Rect) {
    ...
    if ctx.focus_edit_id == id || ctx.active == id {
        ...
        gui_round_stroke(
            ctx,
            inner,
            max(ctx.style.radius_control - inner_width * 0.5, 0),
            ctx.style.accent,
            inner_width,
        )
    }
...
gui_box :: proc(ctx: ^Gui_Context, rect: Rect, style: Gui_Box_Style) {
    ...
    if style.shadow_blur > 0 && style.shadow_color.a > 0 {
        gui_shadow(
            ...
            style.shadow_offset,
            style.shadow_blur,
            gui_apply_opacity(style.shadow_color, opacity),
        )
    }
    if style.gradient {
        append(&ctx.commands, Draw_Command {
            kind    = .Gradient_Rect,
            rect    = rect,
            color   = gui_apply_opacity(style.fill, opacity),
            color_2 = gui_apply_opacity(style.fill_to, opacity),
            radius  = style.radius,
            blend   = style.blend,
        })
    } else if style.fill.a > 0 {
        append(&ctx.commands, Draw_Command {
            kind   = .Filled_Rounded_Rect,
            rect   = rect,
            color  = gui_apply_opacity(style.fill, opacity),
            radius = style.radius,
            blend  = style.blend,
...
gui_lighten :: proc(color: Color, amount: f32) -> Color {
    ...
    return {color.r + (1 - color.r) * t, color.g + (1 - color.g) * t, color.b + (1 - color.b) * t, color.a}
}
...
gui_lerp_color :: proc(a, b: Color, t: f32) -> Color {
    x := gui_clamp01(t)
    return {gui_lerp_f32(a.r, b.r, x), gui_lerp_f32(a.g, b.g, x), gui_lerp_f32(a.b, b.b, x), gui_lerp_f32(a.a, b.a, x)}
}

gui_lerp_vec2 :: proc(a, b: Vec2, t: f32) -> Vec2 {
    x := gui_clamp01(t)
    return {gui_lerp_f32(a.x, b.x, x), gui_lerp_f32(a.y, b.y, x)}
}
...
gui_hint_label :: proc(ctx: ^Gui_Context, x: ^f32, y, h: f32, label: string, gap := true) {
    ...
    gui_text_clipped(
        ctx,
        {x^, y, w, h},
        {x^, y + max((h - ctx.style.body_text_height) * 0.5, 0)},
        label,
...
gui_numeric_tooltip_controller_icon :: proc(ctx: ^Gui_Context, x: ^f32, y, size: f32, slot: int) {
    ...
    gui_image_uv_filtered(
        ctx,
        {x^, y, size, size},
        Gui_Image_Id(UI_KENNEY_INPUT_ATLAS_TEXTURE_ID),
        ctx.style.text,
        {f32(index) / count, 0, 1 / count, 1},
...
gui_draw_notice_overlay :: proc(ctx: ^Gui_Context) {
    if ctx.notice_text_len <= 0 ||
       ctx.notice_seconds <= 0 ||
       ctx.input.window_width <= 0 ||
       ctx.input.window_height <= 0 {
        return
    }
    ...
    gui_round_stroke(
        ...
        gui_apply_opacity(ctx.style.accent, 0.72 * alpha),
        max(ctx.style.border_width, f32(1)),
    )
...
gui_overlay_nudge_into_view :: proc(ctx: ^Gui_Context, rect: Rect) -> Rect {
    ...
    viewport := Rect {
        margin,
        margin,
        max(f32(ctx.input.window_width) - margin * 2, 0),
        max(f32(ctx.input.window_height) - margin * 2, 0),
    }
