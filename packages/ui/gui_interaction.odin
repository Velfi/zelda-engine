...
gui_rect_point_to_normalized :: proc(rect: Rect, point: Vec2) -> Vec2 {
    return {gui_clamp01((point.x - rect.x) / max(rect.w, 1)), gui_clamp01((point.y - rect.y) / max(rect.h, 1))}
}

gui_normalized_to_rect_point :: proc(rect: Rect, value: Vec2) -> Vec2 {
    return {rect.x + rect.w * gui_clamp01(value.x), rect.y + rect.h * gui_clamp01(value.y)}
}
...
gui_vec2_from_normalized :: proc(value, min_value, max_value: Vec2) -> Vec2 {
    ...
    return {min_value.x + (max_value.x - min_value.x) * n.x, min_value.y + (max_value.y - min_value.y) * n.y}
}
