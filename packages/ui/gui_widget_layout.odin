...
gui_collapsible_begin_keyed :: proc(ctx: ^Gui_Context, label, key: string, open: ^bool) -> bool {
    ...
    if (control.focused && (gui_accept_pressed(ctx) || ctx.input.key_space)) ||
...
gui_expander_chrome :: proc(ctx: ^Gui_Context, bounds: Rect, label: string, id: Gui_Id, open, focused: bool) {
    ...
    if open {
        gui_rect(
            ctx,
            {bounds.x + 3, bounds.y + 7, max(ctx.style.border_width * 2, 2), max(bounds.h - 14, 1)},
            ctx.style.accent,
        )
    }
    t := open ? f32(1) : f32(0)
    ...
    text_rect := gui_inset_edges(
        bounds,
        {left = ctx.style.control_padding * 4, top = 0, right = ctx.style.control_padding, bottom = 0},
    )
    gui_text_clipped(
        ctx,
        text_rect,
        {text_x, bounds.y + max((bounds.h - ctx.style.body_text_height) * 0.5, 0)},
        label,
        label_color,
    )
    if focused {
...
gui_scrollbar :: proc(ctx: ^Gui_Context, viewport: Rect, content_height, scroll: f32) {
    ...
    track := Rect {
        viewport.x + viewport.w - track_w - track_margin,
        viewport.y + track_margin,
        track_w,
        max(viewport.h - track_margin * 2, 0),
    }
    if track.h <= 0 {
        return
    }
