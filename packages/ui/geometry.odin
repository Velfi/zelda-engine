...
gui_place_overlay :: proc(anchor: Rect, size: Vec2, viewport: Rect, preferred := Gui_Overlay_Direction.Above, gap: f32 = 6) -> Rect {
	result := Rect{}
...
gui_cubic_segment_count :: proc(a, b, c, d: Vec2) -> int {
	control_length := math.sqrt((b.x - a.x) * (b.x - a.x) + (b.y - a.y) * (b.y - a.y)) +
