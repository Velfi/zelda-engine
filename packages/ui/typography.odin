...
gui_typography :: proc(style: Gui_Style, role: Gui_Text_Role) -> Gui_Typography {
	switch role {
	case .Display:
		return {.Display, style.display_text_height, style.display_line_height, style.display_char_width, style.display_text_scale}
	case .Heading:
		return {.Display, style.heading_text_height, style.heading_line_height, style.heading_char_width, style.heading_text_scale}
	case .Small:
		return {.Body, style.small_text_height, style.small_line_height, style.small_char_width, style.small_text_scale}
	case .Body:
...
gui_space :: proc(style: Gui_Style, token: Gui_Space) -> f32 {
	switch token {
	case .None:          return 0
	case .Quarter:       return style.spacing_1
	case .Half:          return style.spacing_2
	case .One:           return style.spacing_3
	case .One_And_Half:  return style.spacing_4
	case .Two:           return gui_rhythm(style, 2)
	case .Three:         return gui_rhythm(style, 3)
	}
	return 0
}
