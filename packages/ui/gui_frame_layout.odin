...
gui_init_text_shaper :: proc() {
    ...
    for font_kind in Gui_Font_Kind {
        kind := int(font_kind)
        if kind < 0 || kind >= len(gui_font_fallback_counts) || !gui_text_shaper_font_ready[kind] {
            continue
        }
...
gui_begin_frame :: proc(ctx: ^Gui_Context, input: Input_State) {
    ...
    if ctx.mouse_initialized {
        ctx.mouse_delta = {
            frame_input.mouse_pos.x - ctx.mouse_prev_pos.x,
            frame_input.mouse_pos.y - ctx.mouse_prev_pos.y,
        }
    } else {
        ctx.mouse_delta = {}
...
gui_input_apply_keyboard_fallbacks :: proc(input: ^Input_State, previous: Input_State) {
    ...
    input.primary_pressed =
        input.primary_pressed || (input.mouse_pressed && input.mouse_button != 2 && input.mouse_button != 3)
    input.primary_released =
        input.primary_released || (input.mouse_released && input.mouse_button != 2 && input.mouse_button != 3)
...
gui_end_frame :: proc(ctx: ^Gui_Context) {
    ...
    if ctx.focus_edit_id != GUI_ID_NONE &&
       (!ctx.focus_edit_seen ||
               ctx.focused != ctx.focus_edit_id ||
               !gui_spatial_item_registered(ctx, ctx.focus_edit_id)) {
...
gui_panel_begin :: proc(ctx: ^Gui_Context, bounds: Rect) {
    gui_shadow(
        ctx,
        bounds,
        ctx.style.radius_panel,
        ctx.style.shadow_offset,
        ctx.style.shadow_blur,
        ctx.style.shadow_color,
    )
