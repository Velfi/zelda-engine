...
Text_Span :: struct {text:string,effect:Text_Effect}
...
text_effect_lerp :: proc(a,b:Text_Effect,t:f32)->Text_Effect {
	...
	result.bold=amount<.5?a.bold:b.bold;result.italic=amount<.5?a.italic:b.italic;result.underline=amount<.5?a.underline:b.underline
	result.shadow_offset={a.shadow_offset.x+(b.shadow_offset.x-a.shadow_offset.x)*amount,a.shadow_offset.y+(b.shadow_offset.y-a.shadow_offset.y)*amount}
	result.wave_amplitude=a.wave_amplitude+(b.wave_amplitude-a.wave_amplitude)*amount;result.wave_frequency=a.wave_frequency+(b.wave_frequency-a.wave_frequency)*amount;result.wave_speed=a.wave_speed+(b.wave_speed-a.wave_speed)*amount
	result.shake=a.shake+(b.shake-a.shake)*amount;result.pulse_amount=a.pulse_amount+(b.pulse_amount-a.pulse_amount)*amount;result.pulse_speed=a.pulse_speed+(b.pulse_speed-a.pulse_speed)*amount
	result.drift={a.drift.x+(b.drift.x-a.drift.x)*amount,a.drift.y+(b.drift.y-a.drift.y)*amount}
	result.typewriter_characters_per_second=a.typewriter_characters_per_second+(b.typewriter_characters_per_second-a.typewriter_characters_per_second)*amount;result.typewriter_delay=a.typewriter_delay+(b.typewriter_delay-a.typewriter_delay)*amount
	return result
}
...
text_effect_visible_glyphs :: proc(effect:Text_Effect,elapsed:f32,glyph_count:int)->int {
	...
	return clamp(int(max(elapsed-effect.typewriter_delay,0)*effect.typewriter_characters_per_second),0,glyph_count)
}
...
text_effect_hash :: proc(index:int)->f32 {
	value:=u32(index)*747796405+2891336453;shift:=(value>>28)+4;value=((value>>shift)~value)*u32(277803737);value=(value>>22)~value
