...
gui_selector_keyed :: proc(ctx: ^Gui_Context, label, key: string, current: ^int, options: []string) -> bool {
    ...
    } else if !ctx.controller_explicit_activation &&
       center_control.focused &&
       (ctx.input.nav_pressed_x != 0 || ctx.input.nav_pressed_y != 0) {
...
gui_checkbox_keyed :: proc(ctx: ^Gui_Context, label, key: string, value: ^bool) -> bool {
    ...
    if value^ {
        ...
        gui_line(
            ctx,
            {box.x + box.w * 0.23, box.y + box.h * 0.52},
            {box.x + box.w * 0.43, box.y + box.h * 0.72},
            check_color,
            max(ctx.style.border_width * 2, 2),
        )
        gui_line(
            ctx,
            {box.x + box.w * 0.43, box.y + box.h * 0.72},
            {box.x + box.w * 0.78, box.y + box.h * 0.28},
            check_color,
            max(ctx.style.border_width * 2, 2),
        )
    }
    ...
    gui_text_clipped(
        ctx,
        gui_inset_edges(bounds, {left = box_size + 20, top = 0, right = 8, bottom = 0}),
        {bounds.x + box_size + 24, bounds.y + max((bounds.h - ctx.style.body_text_height) * 0.5, 0)},
        label,
        ctx.style.text,
    )
    return clicked
...
gui_switch_keyed :: proc(ctx: ^Gui_Context, label, key: string, value: ^bool) -> bool {
    ...
    gui_shadow(
        ...
        knob,
        knob_size * 0.5,
        {0, max(ctx.style.border_width, f32(1))},
        ctx.style.shadow_blur * 0.35,
        {0, 0, 0, 0.26},
    )
    ...
    gui_text_clipped(
        ctx,
        gui_inset_edges(bounds, {left = label_left - bounds.x, top = 0, right = 8, bottom = 0}),
        {label_left, bounds.y + max((bounds.h - ctx.style.body_text_height) * 0.5, 0)},
        label,
        ctx.style.text,
    )
    return clicked
...
gui_radio_keyed :: proc(ctx: ^Gui_Context, label, key: string, selected: bool) -> bool {
    ...
    gui_text_clipped(
        ctx,
        gui_inset_edges(bounds, {left = size + 20, top = 0, right = 8, bottom = 0}),
        {bounds.x + size + 24, bounds.y + max((bounds.h - ctx.style.body_text_height) * 0.5, 0)},
        label,
        ctx.style.text,
    )
    return clicked
...
gui_radio_group_keyed :: proc(ctx: ^Gui_Context, label, key: string, current: ^int, options: []string) -> bool {
    ...
    for option, i in options {
        ...
        gui_ellipse_stroke(
            ...
            circle,
            border,
            current^ == i ? max(ctx.style.border_width * 2, 2) : ctx.style.border_width,
        )
        if current^ == i {
            gui_ellipse(ctx, gui_inset(circle, max(size * 0.30, f32(6))), ctx.style.accent)
        }
...
gui_area_slider_f32_keyed :: proc(
    ctx: ^Gui_Context,
    label, key: string,
    value: ^Vec2,
    min_value, max_value: Vec2,
) -> bool {
    bounds := gui_next_rect(ctx, height = 160)
    gui_text_clipped(
        ctx,
        {bounds.x, bounds.y, bounds.w, ctx.style.row_height},
...
gui_hue_wheel_at :: proc(ctx: ^Gui_Context, id: Gui_Id, bounds: Rect, hsv: ^Hsv_Color) -> bool {
    ...
    gui_shader_rect(
        ctx,
        {center.x - outer, center.y - outer, outer * 2, outer * 2},
        .Hue_Wheel,
        {inner / outer, 1, 0, hsv.a},
        {1, 1, 1, 1},
    )
    ...
    angle := h * GUI_TAU
    handle := Vec2 {
        center.x + math.cos(angle) * ((inner + outer) * 0.5),
        center.y + math.sin(angle) * ((inner + outer) * 0.5),
    }
    delta := Vec2{ctx.input.mouse_pos.x - center.x, ctx.input.mouse_pos.y - center.y}
    dist := math.sqrt(delta.x * delta.x + delta.y * delta.y)
    hovered :=
        gui_mouse_in_input_clip(ctx) &&
        ((dist >= inner && dist <= outer) || gui_contains_circle(handle, ctx.input.mouse_pos, 10))
    if hovered {
        ctx.hot = id
        if ctx.input.mouse_pressed {
            ctx.active = id
