...
gui_text_wrap_paragraph :: proc(
    ctx: ^Gui_Context,
    bytes: []u8,
    start, end: int,
    max_width: f32,
    lines: ^[dynamic]Gui_Text_Line,
) {
    if end <= start {
...
gui_text_wrap_candidates :: proc(
    ctx: ^Gui_Context,
    bytes: []u8,
    start, end: int,
    max_width: f32,
    out: ^[dynamic]Gui_Wrap_Candidate,
) {
...
gui_text_wrap_forced_candidates :: proc(
    ctx: ^Gui_Context,
    bytes: []u8,
    start, end: int,
    max_width: f32,
...
gui_text_wrap_greedy_fallback :: proc(
    ctx: ^Gui_Context,
    bytes: []u8,
    start, end: int,
    max_width: f32,
...
gui_font_text_width :: proc(font_kind: Gui_Font_Kind, bytes: []u8, scale, fallback_advance: f32) -> f32 {
    ...
    if cacheable {
        ...
        if gui_text_width_cache_matches(
            slot,
            effective_font_kind,
            bytes,
            hash,
...
gui_text_width_cache_matches :: proc(
    entry: ^Gui_Text_Width_Cache_Entry,
    font_kind: Gui_Font_Kind,
    bytes: []u8,
    hash: u64,
    scale, fallback_advance: f32,
    shaper_ready: bool,
) -> bool {
    if !entry.valid ||
       entry.hash != hash ||
       entry.len != len(bytes) ||
       entry.scale != scale ||
       entry.fallback_advance != fallback_advance ||
       entry.font_kind != font_kind ||
       entry.shaper_ready != shaper_ready {
        return false
    }
...
gui_font_render_ascii_atlas :: proc(
    ...
    return(
        vo_textshape_render_ascii_atlas(
            i32(effective_font_kind),
            i32(glyph_first),
            i32(glyph_last),
            i32(pixel_height),
