...
gui_scroll_begin_internal :: proc(
    ctx: ^Gui_Context,
    viewport: Rect,
    content_height: f32,
    scroll: ^f32,
    draggable, animate: bool,
) {
...
gui_spatial_candidate_score :: proc(
    current, candidate: Rect,
    dir_x, dir_y: f32,
) -> (
    valid: bool,
    forward, perpendicular: f32,
    overlap: bool,
) {
...
gui_apply_spatial_navigation :: proc(ctx: ^Gui_Context) {
    ...
    for i in 0 ..< ctx.spatial_item_count {
        ...
        valid, forward, perpendicular, overlap := gui_spatial_candidate_score(
            current.bounds,
            item.bounds,
            dir_x,
            dir_y,
...
gui_apply_tab_navigation :: proc(ctx: ^Gui_Context) {
    if ctx.focus_moved ||
       (!ctx.input.focus_next && !ctx.input.focus_prev) ||
       ctx.focus_edit_id != GUI_ID_NONE ||
       ctx.open_panel != GUI_ID_NONE {
        return
    }
...
gui_control :: proc(
    ...
    focusable := true,
    pointer_focus := true,
    interact_during_transition := false,
) -> Gui_Control {
    ...
    hovered :=
        enabled &&
        snapshot_found &&
        (!discontinuous || interact_during_transition) &&
        !gui_semantic_id_was_unstable(ctx, id) &&
        gui_mouse_contains(ctx, interaction_bounds)
    if hovered {
        ctx.hot = id
