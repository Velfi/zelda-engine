...
image_create :: proc(
	...
	if !engine.vk_allocate_memory_checked(ctx, &allocation, allocation_name, &out.memory) {image_destroy(out, ctx); return false}
...
depth_create :: proc(ctx: ^engine.Vk_Context, width, height: u32, out: ^Image, samples: vk.SampleCountFlags = {._1}) -> bool {
	return image_create(ctx, width, height, .D32_SFLOAT, {.DEPTH_STENCIL_ATTACHMENT}, {.DEPTH}, samples, out, "renderer depth")
}

texture_upload_rgba8 :: proc(ctx: ^engine.Vk_Context, pixels: []u8, width, height: int, out: ^Image, options := Sampler_Options{address_mode=.REPEAT}) -> bool {
	...
	engine.vk_cmd_image_barrier2(ctx, cmd, out.image, {.TOP_OF_PIPE}, {.TRANSFER}, {}, {.TRANSFER_WRITE}, .UNDEFINED, .TRANSFER_DST_OPTIMAL)
	region := vk.BufferImageCopy{imageSubresource = {aspectMask = {.COLOR}, mipLevel = 0, baseArrayLayer = 0, layerCount = 1}, imageExtent = {u32(width), u32(height), 1}}
	vk.CmdCopyBufferToImage(cmd, staging.handle, out.image, .TRANSFER_DST_OPTIMAL, 1, &region)
	engine.vk_cmd_image_barrier2(ctx, cmd, out.image, {.TRANSFER}, {.FRAGMENT_SHADER}, {.TRANSFER_WRITE}, {.SHADER_READ}, .TRANSFER_DST_OPTIMAL, .SHADER_READ_ONLY_OPTIMAL)
