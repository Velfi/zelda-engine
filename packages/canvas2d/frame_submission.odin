...
batch_scissor_rect :: proc(batch: Batch, extent: vk.Extent2D) -> vk.Rect2D {
	...
	y0 := i32(
		clamp(batch.clip.y / logical_height * framebuffer_height, f32(0), framebuffer_height),
	)
	x1 := i32(
		clamp(
			(batch.clip.x + batch.clip.width) / logical_width * framebuffer_width,
			f32(0),
			framebuffer_width,
		),
	)
	y1 := i32(
		clamp(
			(batch.clip.y + batch.clip.height) / logical_height * framebuffer_height,
			f32(0),
			framebuffer_height,
		),
	)
...
publish_screenshot_readback :: proc(
	...
	if engine.screenshot_state_publish_from_gpu_rgba(
		...
		data, _, _, _, encoded := engine.screenshot_state_copy_png(
			&state.capture_state,
...
EndDrawing :: proc() {
	...
	if len(state.vertices) >
	   0 {vb := &state.vertex[frame.frame_index]; ib := &state.index[frame.frame_index]
		upload_marker: u64
		if detailed_gfx do upload_marker = gfx_profile_begin(.UI_Buffer_Upload)
		mem.copy_non_overlapping(
			vb.mapped,
			raw_data(state.vertices[:]),
			len(state.vertices) * size_of(Vertex),
		)
		mem.copy_non_overlapping(
			ib.mapped,
			raw_data(state.indices[:]),
			len(state.indices) * size_of(u32),
		)
		...
		vk.CmdBindPipeline(
			frame.command_buffer,
			.GRAPHICS,
			hdr_active ? state.hdr_pipeline : state.pipeline,
		)
