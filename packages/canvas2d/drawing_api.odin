...
SetWindowSize :: proc(width, height: i32) {_ = render2d.sdl_window_set_size(
		&state.platform_window,
		width,
		height,
	)}
...
keyboard_key_scancodes :: proc(key: KeyboardKey) -> (primary, alternate: sdl.Scancode) {assert(
		key != .COUNT,
	)
	switch
	key {
...
IsKeyPressed :: proc(key: KeyboardKey) -> bool {primary, alternate := keyboard_key_scancodes(key)
	return(
		state.keys_pressed[int(primary)] ||
		alternate != .UNKNOWN && state.keys_pressed[int(alternate)] \
	)}
IsKeyDown :: proc(key: KeyboardKey) -> bool {primary, alternate := keyboard_key_scancodes(key)
	return(
		state.keys_down[int(primary)] ||
		alternate != .UNKNOWN && state.keys_down[int(alternate)] \
	)}
gamepad_sdl_button :: proc(button: Gamepad_Button) -> sdl.GamepadButton {switch
	button {case .South:
...
ButtonBehavior :: proc(id: int, r: Rectangle, enabled: bool) -> Button_Interaction {gui_id :=
		ui.Gui_Id(id + 1)
...
DrawRectangleRounded :: proc(r: Rectangle, roundness: f32, segments: i32, color: Color) {rect(
		r,
		color,
	)}
DrawRectangleRoundedLinesEx :: proc(
	...
	thickness: f32,
	color: Color,
) {rect({r.x, r.y, r.width, thickness}, color); rect(
...
DrawRibbonHatched :: proc(
	...
	for i in 0 ..< point_count - 1 {
		...
		normal_a := Vector2 {
			-tangent_a.y / length_a * half_widths[i],
			tangent_a.x / length_a * half_widths[i],
		}
		normal_b := Vector2 {
			-tangent_b.y / length_b * half_widths[i + 1],
			tangent_b.x / length_b * half_widths[i + 1],
		}
...
DrawCircleV :: proc(center: Vector2, radius: f32, color: Color) {segments := 16; c := transform(
		center,
	)
	for i := 0; i < segments; i += 1 {
		...
		p1 := transform(
			{center.x + radius * f32(math.cos(f64(a))), center.y + radius * f32(math.sin(f64(a)))},
		)
		p2 := transform(
			{center.x + radius * f32(math.cos(f64(b))), center.y + radius * f32(math.sin(f64(b)))},
		)
		base := u32(len(state.vertices))
		t := to_color(color)
