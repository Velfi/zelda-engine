...
expect_vector_near :: proc(t: ^testing.T, actual, expected: Vector2) {
	testing.expectf(
		t,
		math.abs(actual.x - expected.x) < .0001,
		"x: expected %.4f, got %.4f",
		expected.x,
		actual.x,
	)
	testing.expectf(
		t,
		math.abs(actual.y - expected.y) < .0001,
		"y: expected %.4f, got %.4f",
		expected.y,
		actual.y,
	)
}
...
screenshot_gpu_pixels_encode_as_png :: proc(t: ^testing.T) {
	...
	testing.expect(t, engine.screenshot_state_publish_from_gpu_rgba(
		&screenshot,
		pixels[:],
		2,
		1,
...
textured_quads_clipping_and_camera_transforms_are_characterized :: proc(t: ^testing.T) {
	...
	DrawTexturePro(
		{id = 2, width = 64, height = 32, ready = true},
		{16, 8, 32, 16},
		{12, 23, 6, 7},
	)
	EndScissorMode()
...
sdl_mouse_keyboard_pinch_and_frame_edges_translate_deterministically :: proc(t: ^testing.T) {
	...
	testing.expectf(
		t,
		math.abs(GetMousePinchScale() - 1) < .0001,
		"composed pinch scale should be 1",
	)
