...
falling_body_lands_on_floor :: proc(t: ^testing.T) {
	...
	testing.expectf(
		t,
		position.y > 0.45 && position.y < 0.65,
		"expected resting y near 0.5, got %v",
		position.y,
	)
...
four_wheel_vehicle_suspends_and_moves :: proc(t: ^testing.T) {
	...
	set_vehicle_input(world, vehicle, 1, .25, 0, 0)
	...
	testing.expectf(
		t,
		contact_count >= 3,
		"expected at least three grounded wheels, got %v",
		contact_count,
	)
...
height_field_collides_and_updates :: proc(t: ^testing.T) {
	...
	testing.expectf(
		t,
		position.y > .45 && position.y < .65,
		"expected heightfield rest near .5, got %v",
		position.y,
	)
	...
	testing.expectf(
		t,
		position.y > 1.45 && position.y < 1.65,
		"expected updated rest near 1.5, got %v",
		position.y,
	)
}
