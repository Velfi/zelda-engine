...
resolve_color :: proc(model_name: string, value_index: int, ch: u8, palette: map[u8]i32) -> i32 {
    if color, ok := model_override_color(model_name, value_index); ok {
        return color
    }
    if symbol, sok := model_palette_symbol(model_name, value_index); sok {
        if color, ok := palette[symbol]; ok {
            return color
        }
    }
...
build_palette_lut :: proc(frame: mk.Frame, model_name: string, palette: map[u8]i32) -> [256]dr.Color {
    ...
    for i in 0 ..< len(frame.chars) {
        lut[i] = argb_to_color(resolve_color(model_name, i, frame.chars[i], palette))
    }
    return lut
}

find_model_index :: proc(name: string) -> int {
    query := strings.trim(name, " ")
...
reload :: proc(state: ^Viewer_State, palette: map[u8]i32) {
    ...
    if !ip_ok {
        // Platform helper keeps compile-failure diagnostics valid on both native and js/wasm.
        state.status = compile_failure_status(name)
        return
    }
...
main :: proc() {
    ...
    // Platform helper loads from filesystem on native and from embedded XML on web.
    palette = load_example_palette()

    start_model := initial_model_name()
    ...
    // Keep font in package scope so web wasmification can reuse it across main_update calls.
