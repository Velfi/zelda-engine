...
camera_clip_and_input_transitions :: proc(t: ^testing.T) {
	camera := Camera2D{offset = {10, 20}, target = {1, 2}, zoom = 2}
...
mock_resize :: proc(data: rawptr, width, height: i32) -> bool {state := cast(^Mock_Backend_State)data; state.resized = true; return true}
...
mock_texture :: proc(data: rawptr, texture: Texture, pixels: []u8) -> bool {state := cast(^Mock_Backend_State)data; state.texture_updated = true; return true}
mock_screenshot :: proc(data: rawptr, path: string) -> bool {state := cast(^Mock_Backend_State)data; state.screenshot = true; return true}
...
runtime_lifecycle_resize_submission_texture_and_teardown :: proc(t: ^testing.T) {
	...
	descriptor := Renderer_Descriptor {pipeline = {
		vertex = {"v", .Vertex, "main", "v"},
		fragment = {"f", .Fragment, "main", "f"},
	}}
