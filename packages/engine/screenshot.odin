...
screenshot_state_copy_qoi_resized :: proc(state: ^Screenshot_State, max_width, max_height: u32, scale: f32, output_width, output_height: u32, allocator := context.allocator) -> (data: []u8, width, height: u32, sequence: u64, ok: bool) {
	return screenshot_state_copy_encoded_resized(state, max_width, max_height, scale, output_width, output_height, .QOI, allocator)
}

screenshot_state_copy_png :: proc(state: ^Screenshot_State, allocator := context.allocator) -> (data: []u8, width, height: u32, sequence: u64, ok: bool) {
	return screenshot_state_copy_png_sized(state, 0, 0, 1, allocator)
}

screenshot_state_copy_png_sized :: proc(state: ^Screenshot_State, max_width, max_height: u32, scale: f32, allocator := context.allocator) -> (data: []u8, width, height: u32, sequence: u64, ok: bool) {
	return screenshot_state_copy_png_resized(state, max_width, max_height, scale, 0, 0, allocator)
}

screenshot_state_copy_png_resized :: proc(state: ^Screenshot_State, max_width, max_height: u32, scale: f32, output_width, output_height: u32, allocator := context.allocator) -> (data: []u8, width, height: u32, sequence: u64, ok: bool) {
	return screenshot_state_copy_encoded_resized(state, max_width, max_height, scale, output_width, output_height, .PNG, allocator)
}
...
screenshot_state_copy_encoded_resized :: proc(state: ^Screenshot_State, max_width, max_height: u32, scale: f32, output_width, output_height: u32, encoding: Screenshot_Encoding, allocator := context.allocator) -> (data: []u8, width, height: u32, sequence: u64, ok: bool) {
	sync.mutex_lock(&state.mutex)
	defer sync.mutex_unlock(&state.mutex)
...
screenshot_encode_png :: proc(out: ^bytes.Buffer, rgb: []image.RGB_Pixel, width, height: u32, allocator := context.allocator) -> bool {
