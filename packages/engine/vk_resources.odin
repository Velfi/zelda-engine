...
vk_record_resource_error :: proc(ctx: ^Vk_Context, kind: Resource_Error_Kind, resource: string, requested_bytes, available_bytes: u64) {
...
vk_refresh_memory_budget :: proc(ctx: ^Vk_Context) {
	...
	ctx.caps.memory = vk_query_memory_budget(ctx.physical_device, ctx.caps.supports_memory_budget_ext, ctx.caps.memory.override_fraction)
}
...
gpu_heap_allocation_fits :: proc(heap: Gpu_Memory_Heap, requested_bytes, headroom_bytes: u64) -> (available_bytes: u64, ok: bool) {
...
vk_admit_memory_allocation :: proc(ctx: ^Vk_Context, memory_type: u32, requested_bytes: u64, resource: string, headroom_bytes := GPU_ALLOCATION_HEADROOM_BYTES) -> bool {
...
vk_allocate_memory_checked :: proc(ctx: ^Vk_Context, alloc: ^vk.MemoryAllocateInfo, resource: string, out: ^vk.DeviceMemory) -> bool {
