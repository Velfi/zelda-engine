...
vk_end_frame :: proc(ctx: ^Vk_Context, frame: Vk_Frame) -> bool {
    ...
    wait := vk.SemaphoreSubmitInfo {
        ...
        semaphore = frame.state.image_available,
        stageMask = {.COLOR_ATTACHMENT_OUTPUT},
    }
    command := vk.CommandBufferSubmitInfo {
    ...
    signal := vk.SemaphoreSubmitInfo {
        sType     = .SEMAPHORE_SUBMIT_INFO,
        semaphore = frame.state.render_finished,
...
vk_submit_upload_commands :: proc(ctx: ^Vk_Context) -> bool {
    ...
    command := vk.CommandBufferSubmitInfo {
        sType         = .COMMAND_BUFFER_SUBMIT_INFO,
        commandBuffer = ctx.upload_command_buffer,
        deviceMask    = 1,
    }
    submit := vk.SubmitInfo2 {
        sType                  = .SUBMIT_INFO_2,
...
vk_query_memory_budget :: proc(
    ...
    return gpu_memory_budget_from_heaps(
        sizes[:heap_count],
        usages[:heap_count],
        budgets[:heap_count],
        has_budget,
